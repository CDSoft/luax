...
update_argparse()
{
    ...
    cp "$TMP/argparse/src/argparse.lua" "$ROOT/lib/luax/"
}
...
update_serpent()
{
    ...
    cp "$TMP/serpent-$SERPENT_VERSION/src/serpent.lua" "$ROOT/lib/luax/"
    sed -i -e 's/(loadstring or load)/load/g'                       \
           ...
           "$ROOT/lib/luax/serpent.lua"
    echo "--@LIB" >> "$ROOT/lib/luax/serpent.lua"
}
...
update_cbor()
{
    ...
    cp "$TMP/lua-cbor"/cbor.lua "$ROOT/lib/luax/"
    echo "--@LIB" >> "$ROOT/lib/luax/cbor.lua"

    patch -p1 <<EOF
diff --git a/lib/luax/cbor.lua b/lib/luax/cbor.lua
...
update_dkjson()
{
    ...
    cp "$TMP/$JSON_SCRIPT" "$ROOT/lib/luax/json.lua"

    patch -p1 <<EOF
diff --git a/lib/luax/json.lua b/lib/luax/json.lua
...
+        if type(order) == "function" then order = order(value) end
         ...
         for i = 1, n do
           local k = order[i]
EOF
    rm -f "$ROOT/lib/luax/json.lua.orig"
}
...
update_toml()
{
    ...
    cp "$TMP/tinytoml-$TOML_VERSION/tinytoml.lua" "$ROOT/lib/luax/toml.lua"
    patch -p1 <<EOF
diff --git a/lib/luax/toml.lua b/lib/luax/toml.lua
