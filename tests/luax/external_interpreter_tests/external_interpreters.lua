...
if target == "lua" then
    assert(arg[-2] == "lua")
    assert(arg[-1] == "--")
    assert(arg[0] == BUILD/"tests/luax/ext-lua")
    assert(sys.libc == "lua")
    assert(not pandoc)
...
elseif target == "lua-luax" then
    ...
    assert(arg[-1] == "--")
    assert(arg[0] == BUILD/"tests/luax/ext-lua-luax")
    assert(sys.libc == "gnu")
    assert(not pandoc)
...
elseif target == "luax-key" then
    assert(arg[-2] == "luax")
    assert(arg[-1] == "--")
    assert(arg[0] == BUILD/"tests/luax/ext-luax-key")
    assert(not pandoc)

elseif target == "luax-z" then
    assert(arg[-2] == "luax")
    assert(arg[-1] == "--")
    assert(arg[0] == BUILD/"tests/luax/ext-luax-z")
    assert(not pandoc)

elseif target == "pandoc" then
    assert(arg[-2] == "pandoc lua")
    assert(arg[-1] == "--")
    assert(arg[0] == BUILD/"tests/luax/ext-pandoc")
    assert(sys.libc == "lua")
...
elseif target == "pandoc-luax" then
    ...
    assert(arg[-1] == "--")
    assert(arg[0] == BUILD/"tests/luax/ext-pandoc-luax")
    assert(sys.libc == "gnu")
    assert(pandoc and pandoc.Pandoc)
