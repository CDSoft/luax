...
function M.toAscList(t)
    ...
    table.sort(kvs, function(kv1, kv2)
        local t1, t2 = type(kv1), type(kv2)
        if t1 == t2 then return kv1[1] < kv2[1] else return t1 < t2 end
    end)
    return L(kvs)
end
...
function M.toDescList(t)
    local kvs = {}
    for k, v in pairs(t) do kvs[#kvs+1] = {k, v} end
    table.sort(kvs, function(kv1, kv2) return kv2[1] <= kv1[1] end)
    return L(kvs)
end
