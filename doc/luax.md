...
- `$PREFIX/bin/luax-env`: script to update `PATH` and `LUA_CPATH` to be
  sources in `.bashrc` or `.zshrc`
...
contain tags (e.g. in comments) showing how the script is used by
LuaX: - `--@MAIN`: main script (must be unique) - `--@LOAD`: library
that is `require`’d before the main script is run (stored in a global
variable) - other scripts are libraries that must be explicitly
`require`’d by the main script
