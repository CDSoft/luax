...
local tokens = F{
    "# Ninja file generated by bang (https://"..luax_version.url..")\n",
    command_line_token,
    "\n",
    help_token,
    install_token,
...
-- nil = default rule, table = customized rule, false = no rule
local gen_flags = nil ---@type nil | table | false

function generator(flags)

    if flags == false then
        if gen_flags ~= nil and gen_flags ~= false then
            log.error("generator: cannot disable a previously customized generator rule")
        end
        gen_flags = false
        return
    end

    if gen_flags == false then
        log.error("generator: cannot enable or customize a previously disabled generator rule")
    end

    if flags == nil or flags == true then
        flags = {}
    end

    if type(flags) ~= "boolean" and type(flags) ~= "table" then
    ...
    for k, v in pairs(flags) do
        if k == "implicit_in" or k == "implicit_out" then
            gen_flags[k] = {gen_flags[k] or {}, v}
        elseif gen_flags[k] == nil then
            gen_flags[k] = v
        else
            log.error("generator: ", k, ": multiple flag definition")
        end
    end
end
...
local function generator_rule(args)
    ...
    if gen_flags == false then return end
    ...
    if not deps:null() or not gitdeps:null() then
        gen_flags.implicit_in = flatten{ gen_flags.implicit_in or {}, deps, gitdeps }
            : nub()
    ...
    gen_flags.pool = gen_flags.pool or "console"
